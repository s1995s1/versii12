...
if (/video/g.test(mime)) if ((q.msg || q).seconds > 8) return m.reply('*[❗ИНФО❗] МАКСИМАЛЬНОЕ ВИДЕО ДЛЯ СТИКЕРА НЕ БОЛЕЕ 7 СЕКУНД*')
...
else throw '*[❗ИНФО❗] ИЗВЕНИТЕ, ПРОИЗОШЛА ОШИБКА, НЕ ЗАБУДЬТЕ ОТВЕТИТЬ НА ВИДЕО ИЛИ ИЗОБРАЖЕНИЯ. КОТОРОЕ БУДЕТ ПРЕОБРАЖЕНО В СТИКЕР*'

}}
handler.help = ['stiker (caption|reply media)', 'stiker <url>', 'stikergif (caption|reply media)', 'stikergif <url>']
handler.tags = ['sticker']
handler.command = /^стикер(tic?ker)?(gif)?(wm)?$/i
